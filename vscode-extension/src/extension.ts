...
async function generateTestsFromCollectionFile(): Promise<void> {
  ...
  const useEnvironment = await vscode.window.showQuickPick(['✅ Yes', '❌ No'], {
  ...
  try {
    ...
    if (!collection.info || !collection.item) {
      vscode.window.showErrorMessage(
        '⚠️ Selected file does not appear to be a valid Postman collection'
      );
      return;
    }
  } catch {
...
async function selectOutputDirectory(): Promise<string | undefined> {
  ...
  if (workspaceFolder && defaultOutputDir) {
  ...
  return vscode.window.withProgress(
    ...
    async (progress: vscode.Progress<{ message?: string; increment?: number }>) => {
      try {
        ...
        const choice = await vscode.window.showInformationMessage(successMessage, openFolder);
